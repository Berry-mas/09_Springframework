...
@TableGenerator(
        ...
        table = "tbl_my_sequences",     // 테이블이 생성될 때 번호를 테이블 내에 넣어서 관리
        pkColumnName = "my_seq_member_no"   // 그 번호들의 컬럼명
)
...
//    @GeneratedValue(strategy = GenerationType.TABLE, generator = "member_seq_tbl_generator")
    ...
    @Column(name = "member_id", unique = true, nullable = false, columnDefinition = "varchar(10)")  //  해당 컬럼의 길이를 10자로 제한하고, 타입을 varchar로 지정
...
//    @Transient  // entity에 넣을 때 얘는 일단 제외하는 어노테이션 (이 필드 DB랑 무관해!)
    @Column(name = "phone")
    ...
    @Enumerated(EnumType.STRING)    // EnumType.ORDINAL (Enum 타입의 값을 순번으로 지정할 수 있으나, 추천 x)
