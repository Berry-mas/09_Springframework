...
import com.ohgiraffers.datajpa.menu.dto.CategoryRepository;
...
@Service
@RequiredArgsConstructor    // 이걸 쓰면 @Autowired를 쓰지 않아도 됨
...
//          return MenuDTO.changeMenuDTO(menu);  // 수동 매핑 메서드
        return modelMapper.map(menu, MenuDTO.class);  // ModelMapper 라이브러리
...
//        List<Menu> menuList = menuRepository.findAll(); // 정렬 미적용
    ...
    public void modifyMenu(MenuDTO menuDTO) {
        // menuCode를 통해서 영속화된 변경 대상 정보를 조회
        ...
        foundMenu.modifyMenuName(foundMenu.getMenuName());
    }

    /* 8. delete : 엔터티 객체의 필드 값을 삭제 */
    @Transactional
