...
class ManyToOneServiceTest {
    ...
    @DisplayName("N:1 연관관계 객치 그래프 탐색을 이용한 조회 테스트")
    @Test
    void manyToOneFindTest() {
        ...
        // when
        Menu foundMenu = manyToOneService.findMenu(menuCode);   // 영속화
    ...
    @DisplayName("N:1 연관관게 객체 삽입 테스트")
    @ParameterizedTest
    @MethodSource("getMenuInfo")
    void manyToOnInsertTest(
        ...
        MenuDTO menuInfo = new MenuDTO(
                menuCode, menuName, menuPrice, new CategoryDTO(categoryCode, categoryName, null),
                orderableStatus
        );
        // when
