...
public class AuthService {
    ...
    public TokenResponse login(LoginRequest request) {
        ...
        User user = userRepository.findByUsername(request.getUsername())
                .orElseThrow(() -> new BadCredentialsException("올바르지 않은 아이디를 입력하였습니다."));
        // matches 메소드를 이용해서 비밀번호 검증 (평문, 암호화된 값)
        if (!passwordEncoder.matches(request.getPassword(), user.getPassword())) {
            throw new BadCredentialsException("올바르지 않은 비밀번호 입력하였습니다.");
        }

        // 로그인 성공 시 token 발급
        // accessToken, refreshToken
        String accessToken = jwtTokenProvider.createToken(user.getUsername(), user.getRole().name());
        String refreshToken = jwtTokenProvider.createToken(user.getUsername(), user.getRole().name());

        // refreshToken은 서버측에서 관리되어야 하는 데이터이고 성능상 추천되는 환경은 Redis
        ...
        refreshTokenRepository.save(tokenEntity);   // refreshToken을 DB에 저장
    ...
    public void logout(String refreshToken) {
        // refreshToken의 서명 및 만료 검증
        jwtTokenProvider.validateToken(refreshToken);
        String usename = jwtTokenProvider.getUsernameFromJWT(refreshToken);
        refreshTokenRepository.deleteById(usename);
    }
}
