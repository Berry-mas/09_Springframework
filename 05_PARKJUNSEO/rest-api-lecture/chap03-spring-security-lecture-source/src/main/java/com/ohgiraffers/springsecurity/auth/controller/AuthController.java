...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<TokenResponse>> refreshToken(
        ...
        TokenResponse tokenResponse = authService.refreshToken(refreshToken);   // refreshToken이 있으면 검증만 하면 됨
        return buildTokenResponse(tokenResponse);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> logout(
        ...
        ResponseCookie deleteCookie = createDeleteRefreshTokenCookie(); // 만료용 쿠키 생성 (만료된 쿠키로 덮어쓰기 함)
        return ResponseEntity.ok()
                .header(HttpHeaders.SET_COOKIE, deleteCookie.toString())
                .body(ApiResponse.success(null));
    }

    /* accessToken과 refreshToken을 body와 쿠키에 담아서 반환 */
