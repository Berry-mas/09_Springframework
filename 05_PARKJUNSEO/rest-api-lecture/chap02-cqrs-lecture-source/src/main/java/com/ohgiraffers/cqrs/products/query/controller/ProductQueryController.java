...
@RestController
@RequiredArgsConstructor    // private final ProductQueryService productQueryService; 얘와 합쳐지면서 생성자 주입이 됨
@Slf4j
public class ProductQueryController {
    ...
    public ResponseEntity<ApiResponse<ProductDetailResponse>> getProduct(@PathVariable Long productCode) {
        ...
        ProductDetailResponse responsse = productQueryService.getProduct(productCode);
        return ResponseEntity.ok(ApiResponse.success(responsse));
    }

    @GetMapping("/products")    // ProductListResponse에는 물건의 목록이 들어가 있음
